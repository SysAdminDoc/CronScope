...
function layout(parts, opts) {
  const d = dialect(opts.dialect);
  if (opts.fieldMode !== "auto") {
    if (parts.length !== Number(opts.fieldMode)) return null;
    if (opts.fieldMode === "5") return { hasSeconds:false, hasYear:false, minute:0, hour:1, dom:2, month:3, dow:4 };
    if (opts.fieldMode === "6" && d.awsYear) return { hasSeconds:false, hasYear:true, minute:0, hour:1, dom:2, month:3, dow:4, year:5 };
    if (opts.fieldMode === "6") return { hasSeconds:true, hasYear:false, second:0, minute:1, hour:2, dom:3, month:4, dow:5 };
...
export function parseCron(expression, inputOptions = {}) {
  ...
  if (raw.startsWith("@")) {
    ...
    const expanded = opts.fieldMode === "6" ? "0 " + alias : opts.fieldMode === "7" ? "0 " + alias + " *" : alias;
  ...
  if (d.requiresQuestion && ((domRestricted && dowRestricted) || (parsed.domField.noSpecific && parsed.dowField.noSpecific))) return null;
  return parsed;
}
